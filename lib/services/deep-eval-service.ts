...
export const deepEvalService = {
  async analyzeDeepEval(
    file: unknown,
    jobRole?: string,
    company?: string,
    jobDescription?: string
    ...
    try {
      ...
      // GitHub Enrichment (non-blocking)
      ...
      if (!result.success) {
        logger.error("[DeepEval] AI structured generation failed:", result.error);
        return {
          data: null,
      ...
      const evalResult: DeepEvalResult = {
        ...d,
      ...
      return { data: evalResult };
    } catch (error: unknown) {
      logger.error("[DeepEval] Service Error:", error);
      return {
        data: null,

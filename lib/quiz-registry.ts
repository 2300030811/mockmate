...
export function validateQuizRegistry(): void {
  ...
  for (const config of CATEGORY_CONFIGS) {
    ...
    // ponytail: question URLs are server-only; missing URLs degrade gracefully at fetch time.
    if (typeof window === "undefined" && process.env.NODE_ENV === "production" && !config.questionsUrl) {
      ...
      console.warn(`[quiz-registry] ${envKey} not set for '${config.id}'. Quiz will fall back to DB/empty.`);
    }
  }
}

// Auto-run validation on module load — log-only, never crash the bundle
try {

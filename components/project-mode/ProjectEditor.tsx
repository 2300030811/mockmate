...
export function ProjectEditor({ project }: ProjectEditorProps) {
  ...
  const sandpackFiles = Object.entries(project.files).reduce((acc: any, [path, content]) => {
    acc[path] = {
      code: content,
  ...
  return (
    <SandpackProvider 
        ...
        files={sandpackFiles}
        options={{
            visibleFiles: [project.activeFile, ...Object.keys(project.files).filter(f => f !== project.activeFile).slice(0, 2)],
            activeFile: project.activeFile,
        }}
    >
        <ProjectEditorContent project={project} toggleTheme={toggleTheme} isDark={isDark} />
    </SandpackProvider>
  );
}

function ProjectEditorContent({ project, toggleTheme, isDark }: { project: ProjectChallenge, toggleTheme: () => void, isDark: boolean }) {
    ...
    const handleVerify = () => {
        if (!project.validationRegex) {
            toast.info("No automated validation for this challenge. If it looks right, you passed!");
            return;
        }
        ...
        if (passed) {
            ...
            try {
                ...
                if (!completed.includes(project.id)) {
                    localStorage.setItem('completedProjects', JSON.stringify([...completed, project.id]));
                }
            } catch (e) {

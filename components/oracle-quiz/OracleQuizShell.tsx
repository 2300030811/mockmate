...
import { useOracleQuiz } from "@/hooks/useOracleQuiz";
import { QuizMode } from "@/types";
import { useTheme } from "@/app/providers";
import { OracleQuestionCard } from "./OracleQuestionCard";
import { OracleQuizResults } from "./OracleQuizResults";
import { motion } from "framer-motion";
import { Button } from "@/components/ui/Button";
import { QuizSkeleton } from "../aws-quiz/QuizSkeleton"; // Reusing Skeleton

import { OracleQuizNavbar } from "./QuizNavbar";
import { OracleQuizSidebar } from "./OracleQuizSidebar";
import { Star } from "lucide-react";
import { BobAssistant } from "../quiz/BobAssistant";

interface OracleQuizShellProps {
  mode: QuizMode;
}

export function OracleQuizShell({ mode }: OracleQuizShellProps) {
  ...
  const {
      ...
      calculateScore,
      checkAnswer
  } = useOracleQuiz({ initialMode: mode });
  ...
  return (
    <div className={`h-screen w-full flex flex-col overflow-hidden transition-colors duration-500 ${
      isDark 
        ? 'bg-gradient-to-br from-gray-950 via-gray-900 to-red-950' 
            ...
            <div className="flex justify-between items-start mb-6">
              <div className="flex-1 mr-4">
                ...
                <div className={`w-full h-2.5 rounded-full ${
                  ...
                  <div 
                    className="bg-gradient-to-r from-red-600 to-orange-600 h-2.5 rounded-full transition-all duration-300" 
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          ...
          >
            ...
            <p className={`mb-8 text-lg ${
              ...
              You have answered <span className="font-bold text-red-500">{(Object.values(userAnswers) as any[]).filter(a => a && a.length > 0).length}</span> out of <span className="font-bold">{questions.length}</span> questions.
            </p>
      ...
      {mode !== 'exam' && (
        <BobAssistant 
            key={currentQ.id}
            question={currentQ}
            customContext="You are Bob, an Oracle expert. Help the user with this Oracle certification question."
        />
      )}
    </div>
  );

...
export function QuizSidebar({
  ...
  const getQuestionStatus = (q: QuizQuestion, index: number) => {
    // Salesforce userAnswers is complex, might be array or string. Checking length or truthiness.
    const isAnswered = userAnswers[Number(q.id)]?.length > 0 || userAnswers[String(q.id)]?.length > 0;
    const isCurrent = currentQuestionIndex === index;
    // Assuming hook uses numbers for marking, but maybe strings.
    const qIdForMark = Number(q.id); 
    const isMarked = markedQuestions.includes(qIdForMark) || markedQuestions.includes(String(q.id) as any);
